...
```groovy
dependencies {
  implementation 'com.braintreepayments.api:paypal-data-collector:4.39.0'
}
```
...
public class ThreeDSecureActivity extends AppCompatActivity implements ThreeDSecureListener {
  ...
  private void performThreeDSecureVerification(CardNonce cardNonce) {
    ...
    threeDSecureClient.performVerification(this, threeDSecureRequest, (paymentAuthRequest, error) -> {
      if (paymentAuthRequest != null) {
        ...
        threeDSecureClient.continuePerformVerification(ThreeDSecureActivity.this, threeDSecureRequest, paymentAuthRequest);
      } else {
        // handle error
      }
    });
  ...
  @Override
  public void onThreeDSecureSuccess(@NonNull ThreeDSecureResult paymentAuthRequest) {
      // send this nonce to your server
