 ...
 * [PaymentActionRequest], and call [submit]. Milestone 4 integrations (e.g. 3D Secure) hook into
    ...
    private fun handleEvent(event: PaymentActionMethodEvent) {
        when (event) {
            ...
            is PaymentActionMethodEvent.ServerActionRequired -> navigateToResult(
                ...
                event.id,
                "${event.serverAction.name} - ${getString(R.string.payment_actions_server_action_note)}"
            )
