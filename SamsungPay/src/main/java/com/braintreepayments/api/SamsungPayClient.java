    ...
    public void isReadyToPay(final Context context, final SamsungIsReadyToPayCallback callback) {
        if (isSamsungPayAvailable()) {
            ...
            getInternalClient(new GetSamsungPayInternalClientCallback() {
                ...
                public void onResult(@Nullable final SamsungPayInternalClient internalClient, @Nullable Exception error) {
                    if (internalClient != null) {
                        internalClient.getSamsungPayStatus(new GetSamsungPayStatusCallback() {
                            @Override
                            public void onResult(@Nullable Integer status, @Nullable Exception statusError) {
                                if (status != null) {
                                    if (status == SPAY_READY) {
                                        internalClient.getAcceptedCardBrands(new GetAcceptedCardBrandsCallback() {
                                            @Override
                                            public void onResult(@Nullable final List<SpaySdk.Brand> spayAcceptedCardBrands, @Nullable Exception error) {
                                                braintreeClient.getConfiguration(new ConfigurationCallback() {
                                                    @Override
                                                    public void onResult(@Nullable Configuration configuration, @Nullable Exception configError) {
                                                        if (configuration != null) {
                                                            ...
                                                            boolean isReadyToPay = !intersection.isEmpty();
