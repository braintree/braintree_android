...
class DeviceInspectorUnitTest {
    ...
    fun getDeviceMetadata_returnsPlatform() {
        val metadata = sut.getDeviceMetadata(context, btConfiguration, "session-id", IntegrationType.CUSTOM)
    ...
    fun getDeviceMetadata_forwardsIntegrationType() {
        val metadata = sut.getDeviceMetadata(context, btConfiguration, "session-id", IntegrationType.CUSTOM)
        assertEquals(IntegrationType.CUSTOM.stringValue, metadata.toJSON().getString("api_integration_type"))
    }
